...
func TestParseInstallConfigURL(t *testing.T) {
	name, u, err := ParseInstallConfigURL(
		"archclash://install-config?name=MySub&url=https%3A%2F%2Fexample.com%2Fa%3Ft%3D1",
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	name2, u2, err := ParseInstallConfigURL("archclash:///install-config?url=https%3A%2F%2Fa.test%2F")
	if err != nil {
		t.Fatal(err)
	}
	if name2 != "" {

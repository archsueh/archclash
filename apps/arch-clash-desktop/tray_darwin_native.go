...
void ArchTrayRegisterMonoPNG(const unsigned char *p, int n);
void ArchTrayConfigureLabels(const char *showWindow, const char *settings, const char *quit, const char *connect);
void ArchTrayStart(void);
void ArchTrayStop(void);
void ArchTraySetConnectTitle(const char *title);
void archTrayDispatch(int op);
*/
import "C"
...
func startAppTray(a *App) {
	...
	C.ArchTrayConfigureLabels(cShow, cSettings, cQuit, cConnect)
...
func trayConnectTitlePoll(stopCh <-chan struct{}) {
	...
	for {
		...
		case <-tick.C:
			...
			C.ArchTraySetConnectTitle(ctitle)
...
//export archTrayDispatch
func archTrayDispatch(op C.int) {
	trayNativeMu.Lock()
	app := trayNativeApp
	trayNativeMu.Unlock()
	if app == nil {
...
//export archTrayOnReady
func archTrayOnReady() {
	trayNativeMu.Lock()
	app := trayNativeApp
	trayNativeUp = true
	trayNativeMu.Unlock()
...
//export archTrayOnStopped
func archTrayOnStopped() {
	trayNativeMu.Lock()
	app := trayNativeApp
	trayNativeUp = false
	trayNativeMu.Unlock()
...
//export archTrayOnToggleConnect
func archTrayOnToggleConnect() {
	trayNativeMu.Lock()
	app := trayNativeApp
	trayNativeMu.Unlock()
	if app == nil {

...
$dst = Join-Path $env:TEMP 'arch_vc_redist.exe'

try {
    (New-Object System.Net.WebClient).DownloadFile($Url, $dst)
}
catch {
    Write-Host "[arch] vc_redist download failed: $($_.Exception.Message)"
    exit 1
}

if (-not (Test-Path -LiteralPath $dst)) {
    Write-Host "[arch] vc_redist.exe missing after download"
    exit 2
}

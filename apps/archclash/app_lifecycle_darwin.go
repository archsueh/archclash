...
/*
void archOnTerminateRequest(void);
*/
import "C"
...
//export archOnTerminateRequest
func archOnTerminateRequest() {
	darwinLifecycleMu.Lock()
...
//export archOnOpenURL
func archOnOpenURL(raw *C.char) {
	u := strings.TrimSpace(C.GoString(raw))
	if u == "" || !strings.HasPrefix(strings.ToLower(u), "archclash:") {
		return
	}
	darwinLifecycleMu.Lock()
	app := darwinLifecycleApp
